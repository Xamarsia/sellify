...
type DeliveryAddressProps = {
  ...
  onChange: (address: DeliveryAddress, isValid: boolean, isSetAsDefault: boolean) => void;
};
...
export default function DeliveryAddressForm({
  ...
  const onAddressChangeHandler = useCallback((e: ChangeEvent<HTMLInputElement>): void => {
    e.preventDefault();
    setAddress(e.target.value);
  }, [setAddress]);

  const onChangeAddressForm = useCallback((e: FormEvent<HTMLFormElement>): void => {
    e.preventDefault();
  ...
  const onUseAsDefaultChangeHandler = useCallback((e: ChangeEvent<HTMLInputElement>): void => {
  ...
  return (
    ...
    >
      ...
      />
      <Checkbox label="Use as my default address" checked={useAsDefault} onChange={onUseAsDefaultChangeHandler} />
    </form>
  );
}
