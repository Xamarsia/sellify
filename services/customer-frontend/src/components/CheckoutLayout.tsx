...
import { CartItem, ContactInfo, DeliveryAddress, OrderRequest } from "@sellify/customer-ui-components/types";

import { CheckoutStep } from "enums";
import { AlertContext, AlertDialogContent, ProgressBarContent } from "types";

import CheckoutProgressBar from "./CheckoutProgressBar";
import CheckoutReviewForm from "./forms/CheckoutReviewForm";
...
import { changeCartItemQuantity, getCartItems, removeCartItem } from "common/actions/cart-actions";
import {
    getDefaultContactInfo, getDefaultDeliveryAddress,
    updateDefaultContactInfo, updateDefaultDeliveryAddress
...
export default function CheckoutLayout({
    ...
    const [paymentProvider, setPaymentProvider] = useState<PaymentProviderType>(PaymentProvider.Balance);
    ...
    const [isPaymentProviderValid, setIsPaymentProviderValid] = useState<boolean>(true);

    const [isDefaultContactInfo, setDefaultContactInfo] = useState<boolean>(true);
    const [isDefaultDeliveryAddress, setDefaultDeliveryAddress] = useState<boolean>(true);

    const [currentStep, setCurrentStep] = useState<CheckoutStep>(CheckoutStep.SELECTED_PRODUCTS);
    ...
    const onOrder = useCallback((): void => {
        ...
        const alertDialogContent: AlertDialogContent = {
            ...
            description: "Your order hasn't shipped yet, but we will send you and email when it done.",
            controlPanel: <>
    ...
    }, [contactInfo, deliveryAddress, paymentProvider, cartItems, deliveryFee, itemsSubtotalPrice, totalPrice]);


    const onDeliveryAddressChange = useCallback((address: DeliveryAddress, isValid: boolean, isSetAsDefault: boolean): void => {
        setDeliveryAddress(address);
        setDefaultDeliveryAddress(isSetAsDefault)
        setIsDeliveryAddressValid(isValid);
    }, []);

    const onContactInfoChange = useCallback((info: ContactInfo, isValid: boolean, isSetAsDefault: boolean): void => {
        setContactInfo(info);
        setDefaultContactInfo(isSetAsDefault);
        setIsContactInfoValid(isValid);
    }, []);

    const onPaymentProviderChange = useCallback((provider: PaymentProviderType, isValid: boolean): void => {
    ...
    const currentStepContent = useMemo<ProgressBarContent | undefined>(() => {
        const contents = new Map<CheckoutStep, ProgressBarContent>([
            ...
            [CheckoutStep.DELIVERY_INFO, {
                ...
                content: <>
                    <FormSection title="Contact Information">
                        <ContactInfoForm contactInfo={contactInfo} onChange={onContactInfoChange} />
                    </FormSection>
                    <FormSection title="Delivery address">
                        <DeliveryAddressForm deliveryAddress={deliveryAddress} onChange={onDeliveryAddressChange} />
                    </FormSection>
                </>
            }],
            [CheckoutStep.PAYMENT_METHOD, {
                isValid: isPaymentProviderValid,
                content: <FormSection title="Select a payment Method">
                    <PaymentMethodForm
            ...
            [CheckoutStep.REVIEW, {
                isValid: cartItems.length > 0 && isDeliveryAddressValid && isContactInfoValid && isPaymentProviderValid,
    ...
    }, [cartItems, contactInfo, deliveryAddress, paymentProvider, currentStep, isPaymentProviderValid, isDeliveryAddressValid, isContactInfoValid]);
        ...
        <div className="flex w-full flex-col gap-9 ">
            ...
            <div className="flex w-full justify-between gap-12 xl:gap-16 xl:flex-row flex-col">
                <div className="flex w-full flex-col gap-12 xl:gap-16">
                    <CheckoutProgressBar currentStep={currentStep} onStepClick={setCurrentStep} />
                    <div className="flex w-full flex-col divide-y divide-stroke gap-6">
                        {currentStepContent?.content}
                    </div>
                </div>
                <div className="xl:w-min">
                    <OrderSubtotal itemsSubtotal={itemsSubtotalPrice} totalPrice={totalPrice} deliveryFee={5} >
                        <Button fill="parent" disabled={!currentStepContent?.isValid} onClick={onNextStep}>
                            {currentStep < CheckoutStep.REVIEW ? "Confirm and Proceed" : "Final Confirm"}
                        </Button>
                    </OrderSubtotal>
                </div>
            </div>
