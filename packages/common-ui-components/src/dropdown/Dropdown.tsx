...
export default function Dropdown({ title, items, selectedKey, isExtended, disabled, setIsExtended, onItemSelected }: DropdownProps) {
    ...
    return (
        <div className='relative' ref={dropdown}>
            <button
                ...
                disabled={disabled}
                className={`flex items-center px-6 gap-x-4 h-13 enabled:cursor-pointer disabled:cursor-not-allowed
            ...
            {isExtended && <div className='absolute left-0 top-12 w-56 rounded-lg bg-white border border-stroke p-4 flex-col min-h-12 max-h-60 overflow-y-auto z-10'>
                {[...items].map(([key, value]) => {
                    return <DropdownItem key={key} value={key} label={value} selected={key == selectedKey} onItemSelected={onItemSelected} />
                })}
            </div>}
