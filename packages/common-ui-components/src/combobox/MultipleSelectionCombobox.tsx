...
export default function MultipleSelectionCombobox({ title, items, selectedItems, required, disabled, onItemSelected, onItemRemoved }: MultipleSelectionComboboxProps) {
    const dropdown = useRef<HTMLDivElement>(null);
    const [suggestedItems, setSuggestedItems] = useState<Map<string, string>>(items);
    const [isExtended, setIsExtended] = useState<boolean>(false);
    const [query, setQuery] = useState<string>("");


    // returns not selected items with query as substring
    const getSuggestedItems = useCallback((query: string): Map<string, string> => {
        const lowerCaseQuery: string = query.toLowerCase();

        return new Map(
            [...items].filter(([key, value]) => {
                const lowerCaseValue: string = value.toLowerCase();
                return lowerCaseValue.includes(lowerCaseQuery) // query is substring 
                    && !selectedItems.has(key); // only not selected items unique elements are allowed
            })
        );
    }, [items, selectedItems]);


    const onSelected = useCallback((key: string, value: string) => {
        setIsExtended(false);
        setQuery("");
    ...
    const onValueChange = useCallback((e: ChangeEvent<HTMLInputElement>): void => {
        e.preventDefault();
    ...
    return (
        <div className='flex flex-col w-full'>
            <label className={`label text-black m-1 ${required && "after:content-['*'] after:ml-0.5"}`} >
                {title}
            </label>
                ...
                <div className='w-full flex p-4 gap-2 rounded-lg border border-stroke has-focus:border-black has-enabled:hover:border-black'>
                    ...
                    <div className='flex flex-auto flex-wrap gap-2'>
                        {[...selectedItems].map(([key, value]) => {
                            return <ComboboxItem key={key} value={key} label={value} onRemove={onItemRemoved} disabled={disabled} />
                        })}
                ...
                {isExtended &&
                    <div className='absolute w-full rounded-lg bg-white border border-stroke p-4 flex-col min-h-12 max-h-60 overflow-y-auto z-10'>
                        {[...suggestedItems].map(([key, value]) => {
                            return <DropdownItem key={key} value={key} label={value} onItemSelected={onSelected} />
                        })}
                    </div>}
            </div>
        </div>
