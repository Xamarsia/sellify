...
export default function Combobox({ title, items, value, required, disabled, onItemSelected }: ComboboxProps) {
    const [suggestedItems, setSuggestedItems] = useState<Map<string, string>>(items);
    const [isExtended, setIsExtended] = useState<boolean>(false);
    const [query, setQuery] = useState<string>("");
    const dropdown = useRef<HTMLDivElement>(null);

    const onSelected = useCallback((key: string, newValue: string) => {
        setIsExtended(false);
    ...
    const isSelected = useCallback((query: string): undefined | [string, string] => {
        const lowerCaseQuery: string = query.toLowerCase();
    ...
    return (
        <div className="flex flex-col w-full">
            ...
            <div ref={dropdown} className="relative">
                <div ref={dropdown} className="h-13 w-full flex flex-auto p-4 gap-2 rounded-lg border border-stroke has-focus:border-black has-enabled:hover:border-black">
                    <input
                        type="text"
                    ...
                    <TransparentIconButton onClick={onDropdownClick}
                        disabled={disabled}
                    >
