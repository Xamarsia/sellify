...
export default function MediaInputField({ text, size = 'default', onImageSelected }: Props) {
    const [dragActive, setDragActive] = useState<boolean>(false);


    const onDragLeave = useCallback((e: React.DragEvent<HTMLLabelElement>): void => {
        e.preventDefault();
        setDragActive(false);
    }, []);


    const onDragOver = useCallback((e: React.DragEvent<HTMLLabelElement>): void => {
        e.preventDefault();
        setDragActive(true);
    }, []);


    const onDragEnter = useCallback((e: React.DragEvent<HTMLLabelElement>): void => {
        e.preventDefault();
        setDragActive(true);
    ...
    const onDrop = useCallback((e: React.DragEvent<HTMLLabelElement>): void => {
        e.preventDefault();
        setDragActive(false);
